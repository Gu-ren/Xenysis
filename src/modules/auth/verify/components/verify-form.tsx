...
const SIGNUP_STEPS = [
  { label: 'Identity verified', durationMs: 500 },
  { label: 'Startup profile loaded', durationMs: 450 },
  { label: 'Preparing AI workspace', durationMs: 400 },
];

const LOGIN_STEPS = [
  { label: 'Identity verified', durationMs: 500 },
  { label: 'Session restored', durationMs: 400 },
  { label: 'Loading your projects', durationMs: 350 },
];
...
export function VerifyForm() {
  ...
  const destination = source === 'signup' ? '/founder-session' : '/dashboard';
  const steps = source === 'signup' ? SIGNUP_STEPS : LOGIN_STEPS;
  const initTitle = source === 'signup' ? 'Initializing Founder Session' : 'Restoring your session';
